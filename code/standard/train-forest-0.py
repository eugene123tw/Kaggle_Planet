...
#from net.model.vggnet import vgg19_bn as Net

## global setting ################
SIZE = 224 #256 #288   #256   #128  #112
EXT  = 'jpg'  #'jpg'
...
def do_training():
    ...
    pretrained_file = '/home/eugene/Documents/Kaggle_Planet/data/pretrain/densenet/densenet121-241335ed.pth'
    ...
    # pretrained_file = '/home/eugene/Documents/Kaggle_Planet/data/pretrain/resnet/resnet34-333f7ec4.pth'
    #pretrained_file = '/root/share/project/pytorch/data/pretrain/resnet/resnet50-19c8e357.pth'
    #pretrained_file = None #
    #pretrained_file = '/home/eugene/Documents/Kaggle_Planet/data/pretrain/vgg/vgg19-dcbb9e9d.pth'
    #pretrained_file = '/home/eugene/Documents/Kaggle_Planet/data/pretrain/vgg/vgg19_bn-c79401a0.pth'
    ...
    num_classes = len(CLASS_NAMES)
    batch_size  = 32 #96 #64 #48  #48  #96 #96  #80 #96 #96   #96   #96 #128 #

    train_dataset = KgForestDataset( 'train-40479', #'train-35479', #'train-32479',   #'train-32479-unsuper-31936' , #'train-3000',#'train-8000',#'debug-32', #'train-3000',#'train-8000',###
                                    transform=[
                                        lambda x: augment(x),
                                    ...
                                    is_preload=False,
                                    label_csv= 'train_v2.csv' #'train_v2-unsuper-31936.csv' #'train_v2.csv'
                                    )
    ...
    test_dataset = KgForestDataset('train-5000', #'debug-32', #'train-3000',#'valid-8000',
                                    transform=[
                                        lambda x: img_to_tensor(x),
                                    ],
                                    height=SIZE, width=SIZE, ext=EXT,
    ...
    ## optimiser ----------------------------------
    LR = StepLR([ (0,0.1),  (10,0.01),  (25,0.005),  (35,0.001), (40,0.0001), (50,0.00001) ,(51,-1)])
    ## fine tunning
...
def do_thresholds():
    ...
    test_dataset = KgForestDataset( 'train-40479', #'train-35479',  #'valid-8000',  #'train-40479',  #'train-32479', #'train-40479',  #
                                    transform=[
                                         lambda x: img_to_tensor(x),
                                    ],
                                    height=SIZE, width=SIZE, ext=EXT,
...
def do_thresholds_slow_aug():
    ...
    test_dataset = KgForestDataset('train-40479', #'train-32479',  #'train-35479',# 'train-40479', #'valid-8000',  #'train-40479',
                                   transform=[
                                       lambda x: img_to_tensor(x),
                                   ],
                                   height=SIZE, width=SIZE, ext=EXT,
    ...
    for a in range(num_augments):
        ...
        # TODO: Eugene changed
        test_dataset.images = change_images(test_dataset.images, agument)
        test_loss, test_acc, predictions = evaluate_and_predict(net, test_loader)
        all_predictions[a] = predictions
    ...
    for a in range(num_augments + 3):
        ...
        # per class threshold
        init_thresholds = best_thresholds.copy()
        best_thresholds, best_score = find_f_measure_threshold_per_class(predictions, labels,
...
def do_submissions():
    ...
    num_classes  = len(test_dataset.class_names)
    test_num     = test_dataset.num
    # TODO: It's to big!!!! Cannot copy all images to memory!!! Impossible!!!
    test_dataset_images = test_dataset.images.copy()
...
def do_submissions_split1():
    ...
    log = Logger()
    log.open(out_dir+'/submissions/log.submissions.split1.txt',mode='a')
    log.write('\n--- [START %s] %s\n\n' % (datetime.now().strftime('%Y-%m-%d %H:%M:%S'), '-' * 64))
    log.write('\n')

    ## dataset ----------------------------
    log.write('** dataset setting **\n')
    batch_size    = 32 #96 #64 #32 #96 #128

    test_dataset = KgForestDataset( 'test-30000', #'test-61191',  #'valid-8000', #
                                    transform=[
                                         lambda x: img_to_tensor(x),
                                    ],
                                    height=SIZE,width=SIZE,
    ...
    for a in range(num_augments+3):
        ...
        #save
        assert(type(test_loader.sampler)==torch.utils.data.sampler.SequentialSampler)
        np.save(test_dir +'/predictions.split1.npy',predictions)
        np.savetxt(test_dir +'/thresholds.txt',thresholds)
        write_submission_csv(test_dir + '/results.split1.csv', predictions, thresholds, test_dataset.split )
...
def do_submissions_split2():
    ...
    log = Logger()
    log.open(out_dir+'/submissions/log.submissions.split2.txt',mode='a')
    log.write('\n--- [START %s] %s\n\n' % (datetime.now().strftime('%Y-%m-%d %H:%M:%S'), '-' * 64))
    log.write('\n')

    ## dataset ----------------------------
    log.write('** dataset setting **\n')
    batch_size    = 96 #32 #96  #128

    test_dataset = KgForestDataset( 'test-15000', #'test-31191',  #'valid-8000', #
                                    transform=[
                                         lambda x: img_to_tensor(x),
                                    ],
                                    height=SIZE,width=SIZE,
    ...
    for a in range(num_augments+3):
        ...
        #save
        assert(type(test_loader.sampler)==torch.utils.data.sampler.SequentialSampler)
        np.save(test_dir +'/predictions.split2.npy',predictions)
        np.savetxt(test_dir +'/thresholds.txt',thresholds)
        write_submission_csv(test_dir + '/results.split2.csv', predictions, thresholds, test_dataset.split )
...
def do_submissions_split3():
    ...
    log = Logger()
    log.open(out_dir+'/submissions/log.submissions.split3.txt',mode='a')
    log.write('\n--- [START %s] %s\n\n' % (datetime.now().strftime('%Y-%m-%d %H:%M:%S'), '-' * 64))
    log.write('\n')
    ...
    test_dataset = KgForestDataset( 'test-16191', #'test-31191',  #'valid-8000', #
                                    transform=[
                                         lambda x: img_to_tensor(x),
                                    ],
                                    height=SIZE,width=SIZE,
    ...
    for a in range(num_augments+3):
        ...
        #save
        assert(type(test_loader.sampler)==torch.utils.data.sampler.SequentialSampler)
        np.save(test_dir +'/predictions.split3.npy',predictions)
        np.savetxt(test_dir +'/thresholds.txt',thresholds)
        write_submission_csv(test_dir + '/results.split3.csv', predictions, thresholds, test_dataset.split )
...
def do_thresholds_by_voting():
    ...
    out_dir='/home/eugene/Documents/Kaggle_Planet/forest-developer/standard-eugene/results/ensemble'
    test_dir = out_dir +'/ensemble-3/thresholds'
    os.makedirs(test_dir, exist_ok=True)
    ...
    base_dir = '/home/eugene/Documents/Kaggle_Planet/forest-developer'

    dirs = [
         'standard-eugene/results/vgg19-jpg-0/<replace>/average',
         'standard-eugene/results/vgg19-jpg-1/<replace>/average',
         'starter kit -7/results/PyNet_10-jpg-baseline-1/<replace>/average',
        'standard-eugene/results/res34-jpg-0/<replace>/average',
        'standard-eugene/results/res34-jpg-1/<replace>/average',
        'standard-eugene/results/dense121-jpg-1/<replace>/average',
...
def do_submission_by_voting():
    ...
    out_dir = '/home/eugene/Documents/Kaggle_Planet/forest-developer/standard-eugene/results/ensemble'
    test_dir = out_dir + '/ensemble-3/submissions'
...
if __name__ == '__main__':
    ...
    # do_training()
    # do_thresholds()
    # do_thresholds_slow_aug()
    # do_submissions()
    # do_submissions_split1()
    # do_submissions_split2()
    # do_submissions_split3()
